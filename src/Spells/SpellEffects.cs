...
namespace TheDarkestNight
{
    public static partial class SpellEffects
    {
        ...
        public static bool IsSiegeActive()
        {
            ...
            try
            {
                foreach (Agent a in Mission.Current.Agents)
                {
                    ...
                    try { if (a.IsUsingGameObject) return true; } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                }
            }
            ...
            int damageCount, int restoreCount, Team casterTeam)
        {
            ...
            try { TryCastSound(caster.Position, cast.VisualColor); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
            try { TryCastAnimation(caster); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
        }
        ...
        public static void TickHaltedAgents(float dt)
        {
            ...
            foreach (int idx in _haltKeySnap)
            {
                ...
                try { usingEquip = a.IsUsingGameObject; } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                if (remaining <= 0f || usingEquip)
                {
                    _expiredHaltKeys.Add(idx);
                    if (!usingEquip) try { a.SetMaximumSpeedLimit(10f, false); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                }
                else
                {
                    _haltedAgents[idx] = (remaining, frozenPos, srcAgent);
                    if (a.MountAgent == null) try { a.SetMaximumSpeedLimit(0f, false); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                    if (doTeleport && a.MountAgent == null) try { a.TeleportToPosition(frozenPos); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                }
            }
