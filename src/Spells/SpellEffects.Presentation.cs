    ...
    public static partial class SpellEffects
    {
        ...
        public static void TickAnimClears(float dt)
        {
            for (int i = _animClearTimers.Count - 1; i >= 0; i--)
            {
                ...
                if (t <= 0f)
                {
                    ...
                    {
                        bool mounted    = false; try { mounted    = a.MountAgent != null;   } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                        bool usingEquip = false; try { usingEquip = a.IsUsingGameObject;    } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                        if (!mounted && !usingEquip)
                            try { a.SetActionChannel(0, _castAnimClearCache, true, 0UL); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                    }
        ...
        public static void TickCastLoops(float dt)
        {
            for (int i = _castLoops.Count - 1; i >= 0; i--)
            {
                ...
                if (newT <= 0f)
                {
                    ...
                    bool usingEquip = false; try { usingEquip = a.IsUsingGameObject;   } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                    if (!mounted && !usingEquip)
                        try { a.SetActionChannel(0, _castAnimCache, true, 0UL); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
        ...
        public static void TickPendingNpcCasts(float dt)
        {
            for (int i = _pendingNpcCasts.Count - 1; i >= 0; i--)
            {
                ...
                {
                    EndCastLoop(a);
                    try { action(); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                    _pendingNpcCasts.RemoveAt(i);
                }
        ...
        public static void BeginCastLoop(Agent agent)
        {
            ...
            try { if (agent.MountAgent != null) return; } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
            try { if (agent.IsUsingGameObject) return; } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
            ...
            try { agent.SetActionChannel(0, _castAnimCache, true, 0UL); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
        ...
        public static void ClearAnimTimers()
        {
            foreach (var (agent, _) in _animClearTimers)
                if (agent != null && agent.IsActive() && agent.Health > 0f)
                    try { agent.SetActionChannel(0, _castAnimClearCache, true, 0UL); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
