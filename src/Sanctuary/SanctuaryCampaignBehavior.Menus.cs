...
namespace TheDarkestNight
{
    public partial class SanctuaryCampaignBehavior
    {
        private static void RegisterSanctuaryMenus(CampaignGameStarter starter)
        {
            ...
            try
            {
                starter.AddGameMenuOption("town", "sanctuary_enter", "{SANCT_ENTER_TEXT}",
                    ...
                    args => { try { GameMenu.SwitchToMenu("sanctuary_menu"); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); } },
                    false, -1, false);
            }
        ...
        private static void DoPrayForGrace()
        {
            ...
            if (hero == null) { try { GameMenu.SwitchToMenu("sanctuary_menu"); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); } return; }

            // Non-lethal HP cost (EmberCovenant reduces it).
            int hpCost = TalentSystem.Has(TalentId.EmberCovenant) ? 8 : 12;
            try { hero.HitPoints = Math.Max(1, hero.HitPoints - hpCost); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                ...
                try
                {
                    ...
                    if (party?.MemberRoster != null)
                    {
                        ...
                        foreach (var e in party.MemberRoster.GetTroopRoster().ToList())
                        {
                            ...
                            try { party.MemberRoster.AddToCounts(e.Character, 0, false, -heal); totalHealed += heal; } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                        }
            ...
            try
            {
                InformationManager.ShowInquiry(new InquiryData("Prayer for Grace", msg, true, false,
                    "So be it.", "", () => { try { GameMenu.SwitchToMenu("sanctuary_menu"); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); } }, null));
            }
            catch
            {
                MBInformationManager.AddQuickInformation(new TextObject(msg.Length > 80 ? msg.Substring(0, 80) + "…" : msg));
                try { GameMenu.SwitchToMenu("sanctuary_menu"); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
            }
        }
        ...
        private static void DoWardingSeal()
        {
            ...
            try { hero.HitPoints = Math.Max(1, hero.HitPoints - 15); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
            ...
            float px = 0f, py = 0f;
            try { px = MobileParty.MainParty.GetPosition2D.x; py = MobileParty.MainParty.GetPosition2D.y; } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
            ...
            try
            {
                InformationManager.ShowInquiry(new InquiryData("Warding Seal", msg, true, false,
                    "I carry it now.", "", () => { try { GameMenu.SwitchToMenu("sanctuary_menu"); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); } }, null));
            }
            catch
            {
        ...
        private static void DoCommitVigil(GraceTrait trait, int targetLevel, int goldCost, int hpCost, bool payGold)
        {
            ...
            if (payGold)
            {
                ...
                try { hero.ChangeHeroGold(-goldCost); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
            }
            else
            {
                try { hero.HitPoints = Math.Max(1, hero.HitPoints - hpCost); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
            ...
            try { hero.SetTraitLevel(MiracleEffects.TraitObjectOf(trait), targetLevel); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
