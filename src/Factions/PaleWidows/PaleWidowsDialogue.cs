// =============================================================================
// THE DARKEST NIGHT — Factions/PaleWidows/PaleWidowsDialogue.cs
// Replaces vanilla lord dialogue for all Pale Widows lords (Southern Empire
// kingdom, StringId "empire_s") with lines befitting a court of women who
// bought their peace with the blood of the men who could not keep it.
// Mirrors the structure of LegionDialogue.cs / EmpireDialogue.cs. The vassal
// title "Widow" is woven through every pool — a Pale Widows lord (always
// female, see PaleWidowsCampaignBehavior's leadership enforcement) is never
// called "vassal".
//
...
//
// Priority is set one step above LegionDialogue (196) — this pool always
// wins over any earlier registration for the same conversation tokens. Note
// membership is checked by KINGDOM (PaleWidowsCulture.IsPaleWidowLord),
// never by CultureObject — the three Empire successor kingdoms share one
// culture, so an Empire lord and a Pale Widows lord are otherwise
...
{
    internal static class PaleWidowsDialogue
    {
        internal static void Register(CampaignGameStarter starter)
        {
            const int P = 197; // above LegionDialogue (196)

            // Flavour greeting only — route straight into the normal vanilla lord
            // options hub (hero_main_options) so Pale Widows lords keep every
            // standard interaction (tasks, barter, war/peace, recruitment).
            // Gated on HasMet so first-meeting introductions still run through
            // vanilla.
            RegisterPool(starter, "pw_start",    "start",                "hero_main_options", _openings, P, requireMet: true, guardPostBattle: true);
            ...
            RegisterPool(starter, "pw_barter",   "lord_barter_question", "close_window",  _barters,   P);
            RegisterPool(starter, "pw_defeat1",  "defeated_lord_start_1","close_window",  _defeats1,  P);
            RegisterPool(starter, "pw_defeat2",  "defeated_lord_start_2","close_window",  _defeats2,  P);
            RegisterPool(starter, "pw_prisoner", "prisoner_chat",        "close_window",  _prisoners, P);
        }
            ...
            for (int i = 0; i < pool.Length; i++)
            {
                ...
                try
                {
                    starter.AddDialogLine(
                        ...
                        text,
                        () => IsPaleWidowsVariant(variant, pool.Length, requireMet, guardPostBattle),
                        null,
                        priority);
                }
        ...
        private static bool IsPaleWidowsVariant(int variant, int poolSize, bool requireMet, bool guardPostBattle)
        {
            try
            {
                ...
                if (!PaleWidowsCulture.IsPaleWidowLord(h)) return false;
        ...
        {
            "Phycaon still stands because the ledger was paid, Widow. What business brings you to my hall?",
            "We did not survive the Long Night by being gentle about it. Speak plainly.",
            "Every man in Lycaron knows his place now — mine is to rule, and his is to be grateful for it. What do you need?",
            "The dark took what it was owed and left the rest of us in peace. Don't waste that peace on small talk.",
            "A Widow does not flinch from what keeps her city standing. What's your business?",
            "I've buried more husbands than I care to count keeping that gate shut. Say your piece.",
        };
        ...
        private static readonly string[] _defeats1 =
        {
            ...
            "Strike true, then. Phycaon has buried better than you and kept standing.",
        ...
        {
            "A cage won't hold what Phycaon owes me. Ransom me or don't — decide quickly.",
