...
namespace TheDarkestNight
{
    public partial class SeaCampaignBehavior
    {
        ...
        private static void VoyageOnConsequence(MenuCallbackArgs args)
        {
            ...
            catch { try { Arrive(); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); } }
        }
        ...
        private static void Arrive()
        {
            ...
            try
            {
                ...
                if (dest != null)
                {
                    ...
                    try { main.Position = dest.GatePosition; } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                    ...
                    chosen =>
                    {
                        ...
                        switch (pick)
                        {
                            case "burn":
                                ...
                                break;
                            case "part":
                                try { Hero.MainHero.HitPoints = Math.Max(1, Hero.MainHero.HitPoints - SeaMath.FogPartHpCost); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                                MBInformationManager.AddQuickInformation(new TextObject(
                ...
                MBInformationManager.ShowMultiSelectionInquiry(new MultiSelectionInquiryData(
                    ...
                    chosen =>
                    {
                        ...
                        switch (pick)
                        {
                            ...
                            case "sense":
                            {
                                try { AgingSystem.AgeHero(Hero.MainHero, SeaMath.SenseWreckAgingDays); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                            ...
                            case "feel":
                            {
                                try { Hero.MainHero.HitPoints = Math.Max(1, Hero.MainHero.HitPoints - SeaMath.SenseWreckHpCost); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                                _voyageHoursTotal += 1f;
                                int gold = SeaMath.FloatsamGold(_rng.NextDouble()) * 2;
                                if (gold > 0)
                                    try { GiveGoldAction.ApplyBetweenCharacters(null, Hero.MainHero, gold, true); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
            ...
            try
            {
                ...
                MBInformationManager.ShowMultiSelectionInquiry(new MultiSelectionInquiryData(
                    ...
                    chosen =>
                    {
                        ...
                        {
                            case "sear":
                                try { AgingSystem.AgeHero(Hero.MainHero, SeaMath.SearTheTideAgingDays); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                                ResolveBoardingFight(FleetStrengthOf(MobileParty.MainParty, searTheTide: true), corsairStr);
                                break;
                            case "current":
                                try { Hero.MainHero.HitPoints = Math.Max(1, Hero.MainHero.HitPoints - SeaMath.CallCurrentHpCost); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                                ResolveBoardingFight(FleetStrengthOf(MobileParty.MainParty, searTheTide: true), corsairStr);
                                break;
                            case "tribute":
                                try { GiveGoldAction.ApplyBetweenCharacters(Hero.MainHero, null, tribute, true); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                                MBInformationManager.AddQuickInformation(new TextObject(
            ...
            try
            {
                ...
                if (outcome.Victory)
                {
                    if (outcome.LootGold > 0)
                        try { GiveGoldAction.ApplyBetweenCharacters(null, Hero.MainHero, outcome.LootGold, true); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                    try { GainRenownAction.Apply(Hero.MainHero, 3f); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                    InformationManager.ShowInquiry(new InquiryData(
                        "☠  Corsairs — Repelled",
                ...
                else
                {
                    ...
                    if (stolen > 0)
                        try { GiveGoldAction.ApplyBetweenCharacters(Hero.MainHero, null, stolen, true); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                    InformationManager.ShowInquiry(new InquiryData(
                        "☠  Corsairs — Overrun",
        ...
        private static void ResolveBlockadeBattle(float playerStr, float blkStr)
        {
            try
            {
                ...
                if (outcome.Victory)
                {
                    if (outcome.LootGold > 0)
                        try { GiveGoldAction.ApplyBetweenCharacters(null, Hero.MainHero, outcome.LootGold, true); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                    try { GainRenownAction.Apply(Hero.MainHero, 5f); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                    InformationManager.ShowInquiry(new InquiryData(
                        "⚓  Blockade Broken",
        ...
        private static void TurnBackFromBlockade()
        {
            try
            {
                ...
                {
                    var main = MobileParty.MainParty;
                    try { main.Position = origin.GatePosition; } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                    try { main.SetMoveGoToSettlement(origin, MobileParty.NavigationType.Default, false); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                }
                        ...
                        switch (pick)
                        {
                            case "take":
                            {
                                ...
                                if (lord != null)
                                {
                                    ...
                                    try { ChangeRelationAction.ApplyRelationChangeBetweenHeroes(Hero.MainHero, lord, 1, false); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                                    InformationManager.ShowInquiry(new InquiryData(
                                        "🚣  Survivors — Rescued",
                            ...
                            case "feel_life":
                            {
                                try { Hero.MainHero.HitPoints = Math.Max(1, Hero.MainHero.HitPoints - SeaMath.FeelLifeHpCost); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                ...
                MBInformationManager.ShowMultiSelectionInquiry(new MultiSelectionInquiryData(
                    ...
                    chosen =>
                    {
                        ...
                        switch (pick)
                        {
                            ...
                            case "speak":
                            {
                                try { AgingSystem.AgeHero(Hero.MainHero, SeaMath.SerpentAgingDays); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                                AddSeaMorale(2f);
                                InformationManager.ShowInquiry(new InquiryData(
                                    "🐉  The Deep Listens",
                            ...
                            case "commune":
                            {
                                try { Hero.MainHero.HitPoints = Math.Max(1, Hero.MainHero.HitPoints - SeaMath.SerpentCommuneHpCost); } catch (System.Exception logEx) { TheDarkestNight.ModLog.Error(logEx); }
                                AddSeaMorale(2f);
                                InformationManager.ShowInquiry(new InquiryData(
                                    "🐉  The Deep Receives",
